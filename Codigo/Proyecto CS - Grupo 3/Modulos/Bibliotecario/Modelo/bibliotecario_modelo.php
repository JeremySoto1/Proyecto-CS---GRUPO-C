...
function insertarBibliotecario($nombre, $apellido, $email, $usuario, $contrasenia) {
    global $enlace;
    $stmt = mysqli_prepare($enlace, "CALL sp_insertar_bibliotecario(?, ?, ?, ?, ?)");
...
function obtenerBibliotecarios() {
    global $enlace;
    $bibliotecarios = [];
    $stmt = mysqli_prepare($enlace, "CALL sp_obtener_bibliotecarios()");
...
function modificarBibliotecario($bibliotecarioID, $nombre, $apellido, $email, $usuario, $contrasenia = '') {
    global $enlace;
    $stmt = mysqli_prepare($enlace, "CALL sp_modificar_bibliotecario(?, ?, ?, ?, ?, ?)");
...
function eliminarBibliotecario($bibliotecarioID) {
    global $enlace;
    $stmt = mysqli_prepare($enlace, "CALL sp_eliminar_bibliotecario(?)");
...
function buscarBibliotecarios($campo, $valor) {
    global $enlace;
    $bibliotecarios = [];
    $stmt = mysqli_prepare($enlace, "CALL sp_buscar_bibliotecarios(?, ?)");
