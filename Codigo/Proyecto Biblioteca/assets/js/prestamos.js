...
function mostrarMensaje(mensaje, tipo = "error") {
  ...
  if (tipo === "error") {
    contenedor.style.backgroundColor = "#fdecea";
    contenedor.style.color = "#d8000c";
    contenedor.style.borderColor = "#d8000c";
  } else if (tipo === "info") {
    contenedor.style.backgroundColor = "#e7f3fe";
    contenedor.style.color = "#31708f";
    contenedor.style.borderColor = "#bce8f1";
  }
...
function buscarLector() {
  ...
  fetch(
    `../../Controlador/Modulos/prestamo_formulario_controlador.php?cedula=${encodeURIComponent(
      cedula
    )}`
  )
    .then((response) => {
    ...
    .then((data) => {
      if (data && data.id) {
        ...
        mostrarMensaje("Lector no encontrado.");
        resetLectorFields();
      }
    })
    .catch((error) => {
...
function buscarLibros() {
  ...
  fetch(
    `../../Controlador/Modulos/prestamo_formulario_controlador.php?buscarLibro=${encodeURIComponent(
      busqueda
    )}`
  )
    .then((res) => res.json())
    .then((existencias) => {
      ...
      if (!existencias || existencias.length === 0) {
        mostrarMensaje("Existencias no disponibles para este libro.");
        return;
      }
