<?php
require_once __DIR__ . '/../Config/conexion_be.php';//Llamada a la conexión de BD
require_once __DIR__.'/../../Controlador/Helpers/excepciones/excepciones_libro.php'; // Agregamos el archivo de excepciones
require_once __DIR__.'/../../Controlador/Helpers/Servicios/LibroValidatorService.php';
...
function eliminarLibro($libroID) {
    global $enlace;   
    $stmt = mysqli_prepare($enlace, "CALL sp_eliminar_libro(?)"); //Llamada al procedimiento almacenado
...
function obtenerLibrosHabilitados() {
    global $enlace;
    $libros = [];
    $stmt = mysqli_prepare($enlace, "CALL sp_obtener_libros_habilitados()");//Llamada al procedimiento almacenado
    ...
    mysqli_stmt_close($stmt);
    return $libros;//Retorna los valores
}
...
function obtenerLibrosDeshabilitados() {
    global $enlace;
    $libros = [];
    $stmt = mysqli_prepare($enlace, "CALL sp_obtener_libros_deshabilitados()");//Llamada al procedimiento almacenado
    mysqli_stmt_execute($stmt);//Ejecuta el procedimiento
    $resultado = mysqli_stmt_get_result($stmt);
    while ($fila = mysqli_fetch_assoc($resultado)) {
        $libros[] = $fila;
    ...
    mysqli_stmt_close($stmt);
    return $libros;//Retorna los valores consultados
}
...
function buscarLibros($campo_busqueda, $valor_busqueda) {
    global $enlace;
    $libros = [];
    $stmt = mysqli_prepare($enlace, "CALL sp_buscar_libros(?, ?)");//Llamada al procedimiento almacenado
    ...
    mysqli_stmt_close($stmt);
    return $libros;//Retora los valores 
}
?>
